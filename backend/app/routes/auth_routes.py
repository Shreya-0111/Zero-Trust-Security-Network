...
def _clear_auth_cookies(resp):
    ...
    # Cookie rules in browsers:
    # - `SameSite=None` generally requires `Secure` (HTTPS), otherwise the cookie may be rejected.
    # - On `http://localhost` dev, `SameSite=Lax` is typically sufficient and avoids the Secure requirement.
    secure = not IS_DEV
...
def dev_login():
    ...
    try:
        ...
        # Use distinct IDs based on email so dual approval works in local dev
        role = data.get('role') or 'admin'
        email = data.get('email') or ('admin@example.com' if role == 'admin' else 'dev@example.com')
        
        # Generate a unique UID based on email for dev mode
        import hashlib
        email_hash = hashlib.md5(email.lower().encode()).hexdigest()[:12]
        unique_uid = f"dev_{role}_{email_hash}"

        user_data = {
            'uid': data.get('userId') or unique_uid,
            'email': email,
            'name': data.get('name') or (f'Dev Admin ({email})' if role == 'admin' else 'Development User'),
            'role': role,
            'email_verified': True
        }
